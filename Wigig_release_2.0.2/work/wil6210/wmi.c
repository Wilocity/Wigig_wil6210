...
static void wmi_evt_rx_mgmt(struct wil6210_priv *wil, int id, void *d, int len)
{
	...
	if (ieee80211_is_beacon(fc) || ieee80211_is_probe_resp(fc)) {
		...
		/* FIXME: have to set ESS bit unless cfg80211 know
		 * now to handle PBSS in __cfg80211_connect_result()
		 */
		if (use_pcp_for_ap)
...
wil_err(wil, "EREZK !!! wmi_evt_ba_status: evt->ringid %x, req_agg_wsize %d", evt->ringid, evt->agg_wsize);
...
static void wmi_evt_rcp_addba_req(struct wil6210_priv *wil, int id, void *d,
				  int len)
{
	struct wmi_rcp_addba_req_event *evt = d;

	wil_rcp_addba_request(wil, evt->cidxtid, evt->dialog_token,
...
static const struct {
	...
	{WMI_RCP_ADDBA_REQ_EVENTID,	wmi_evt_rcp_addba_req},
};
...
int wmi_pcp_start(struct wil6210_priv *wil, int bi, u8 wmi_nettype, u8 chan)
{
	...
	struct wmi_pcp_start_cmd cmd = {
		...
		.disable_sec_offload = !passphrase,
		.channel = chan-1,
	};
	struct {
		struct wil6210_mbox_hdr_wmi wmi;
...
{
	struct wmi_set_passphrase_cmd cmd = {
		.ssid_len = ssid_len,
		.passphrase_len = pass_len,
	};

	if (ssid_len > sizeof(cmd.ssid))
		return -EINVAL;
	if (pass_len > sizeof(cmd.passphrase))
		return -EINVAL;

	memcpy(cmd.ssid, ssid, ssid_len);
	memcpy(cmd.passphrase, pass, pass_len);

	return wmi_send(wil, WMI_SET_PASSPHRASE_CMDID, &cmd, sizeof(cmd));
}
...
int wmi_p2p_cfg(struct wil6210_priv *wil, int channel, int bi)
{
	...
	if (wil->scan_request) {
		wil_dbg_misc(wil, "P2P cfg: Scan in progress\n");
		return -EAGAIN;
	}
...
int wmi_start_discovery(struct wil6210_priv *wil)
{
	...
	rc = wmi_call(wil, WMI_DISCOVERY_START_CMDID, NULL, 0,
		      WMI_DISCOVERY_STARTED_EVENTID, NULL, 0, 100);
...
int wmi_tx_eapol(struct wil6210_priv *wil, struct sk_buff *skb)
{
	...
	eth = eth_hdr(skb);
	cid = wil_find_cid(wil, eth->h_dest);

	wil_dbg_wmi(wil, "EAPOL %d bytes to %pM, CID %d\n", eapol_len,
		    eth->h_dest, cid);
...
{
	if (on)
		return wmi_start_listen(wil);
	else
		return wmi_stop_discovery(wil);
}
...
int wmi_mgmt_tx(struct wil6210_priv *wil, const u8 *buf, size_t len)
{
	...
	if (rc == 0 && evt.evt.status != WMI_TX_SW_STATUS_SUCCESS)
		rc = -EINVAL; /* TODO: proper error code here */
		       ...
		       u16 status, bool amsdu, u16 agg_wsize, u16 timeout)
{
	...
	struct wmi_rcp_addba_resp_cmd cmd = {
		...
		/* always establish immediate BACK - bit 1 */
		.ba_param_set = cpu_to_le16((amsdu ? 1 : 0) | 2 | (tid << 2) |
					    (agg_wsize << 6)),
		.ba_timeout = cpu_to_le16(timeout),
	};
	struct {
